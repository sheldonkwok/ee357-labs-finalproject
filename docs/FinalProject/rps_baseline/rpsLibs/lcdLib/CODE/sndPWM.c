...
static void SetRatio(void)
{
  TPM2C2V = (word)RatioStore;          /* Calculate new compare value according to the given ratio */
}
...
byte sndPWM_Enable(void)
{
  ...
  /* TPM2SC: TOF=0,TOIE=1 */
  clrSetReg8Bits(TPM2SC, 0x80, 0x40);  /* Clear Overflow interrupt flag and enable Overflow interrupt */ 
...
{
  /* TPM2SC: TOIE=0 */
  clrReg8Bits(TPM2SC, 0x40);           /* Disable Timer Overflow Interrupt */ 
  /* TPM2C2SC: CH2F=0,CH2IE=0,MS2B=0,MS2A=0,ELS2B=0,ELS2A=0,??=0,??=0 */
  setReg8(TPM2C2SC, 0x00);             /* Disable output signal */ 
  /* PTHPF1: H7=0 */
...
byte sndPWM_SetDutyUS(word Time)
{
  ...
  PE_Timer_LngMul((dword)Time, 0x18000000LU, &rtval); /* Multiply given value and High speed CPU mode coefficient */
...
byte sndPWM_SetDutyMS(word Time)
{
  ...
  PE_Timer_LngMul((dword)Time, 0x5DC00000LU, &rtval); /* Multiply given value and High speed CPU mode coefficient */
...
void sndPWM_Init(void)
{
  ...
  /* TPM2C2SC: CH2F=0,CH2IE=0,MS2B=1,MS2A=1,ELS2B=1,ELS2A=1,??=0,??=0 */
  setReg8(TPM2C2SC, 0x3C);             /* Set up PWM mode with output signal level low */ 
  RatioStore = 0x0168U;                /* Store initial value of the ratio */
  sndPWM_EnEvent = FALSE;              /* Disable events */
  /* TPM2CNT: BIT15=0,BIT14=0,BIT13=0,BIT12=0,BIT11=0,BIT10=0,BIT9=0,BIT8=0,BIT7=0,BIT6=0,BIT5=0,BIT4=0,BIT3=0,BIT2=0,BIT1=0,BIT0=0 */
  setReg16(TPM2CNT, 0x00U);            /* Reset counter */ 
  ...
  setReg16(TPM2MOD, 0x00U);            /* Set modulo register to zero - free-running timer counter */ 
