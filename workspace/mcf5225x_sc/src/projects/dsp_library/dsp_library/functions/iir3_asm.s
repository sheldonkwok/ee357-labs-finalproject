...
/*----------------------------------------------------------------------
Description: 3rd order IIR filter. All filter coefficients must be
...
y(n) = (2^-b) * (B3*x(n-3) + B2*x(n-2) + B1*x(n-1) + B0*x(n)) +
       (2^-a) * (A3*y(n-3) + A2*y(n-2) + A1*y(n-1))

----------------------------------------------------------------------
COEFFICIENTS (8x2 Bytes):
offset  {upper, lower}
0       {B3   , B2   }
4       {B1   , B0   }
8       {A3   , A2   }
...
{B3,B2,B1,B0} are 16-bit signed fixed-point numerator coefficients
scaled by b-bits.
{A3,A2,A1,A0} are 16-bit signed fixed-point denominator coefficients
...
{x(n), x(n-1), x(n-2), x(n-3)} are current and previous 16-bit inputs.
{y(n), y(n-1), y(n-2), y(n-3)} are current and previous 16-bit outputs.
...
_iir3_asm:
	...
	movem.l	IIR_BUFFER_OFFSET(A0),D1-D3	; load all previous inputs and outputs
	movem.l	(A2),D4-D6			; load {B3, B2} {B1, B0} {A3, A2}
	swap.w	D0				; swap current input into upper half of reg

	mac.w	D3.u,D4.u			; acc + x(n-3)*B3
	mac.w	D2.u,D4.l			; acc + x(n-2)*B2
	mac.w	D1.u,D5.u			; acc + x(n-1)*B1
	mac.w	D0.u,D5.l			; acc + x(n)*B0	
	...
	move.l	12(A2),D7			; load {A1, A0}

	mac.w	D3.l,D6.u			; acc + y(n-3)*A3
	mac.w	D2.l,D6.l         		; acc + y(n-2)*A2
...
_write_data_out:
	...
	movem.l	D0-D2,IIR_BUFFER_OFFSET(A0)	; advance buffer
